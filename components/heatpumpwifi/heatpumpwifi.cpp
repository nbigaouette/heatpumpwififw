#include "esphome/core/log.h"
#include "heatpumpwifi.hpp"

// gnu++20

namespace esphome
{
    namespace heatpumpwifi
    {
        static const char *const TAG = "HeatPumpWiFi";

        HeatPumpWiFi::HeatPumpWiFi()
        {
            // Do setup here
        }

        // Initialize device last so that logger is initialized and available in `setup()`
        float HeatPumpWiFi::get_setup_priority() const
        {
            return setup_priority::LATE;
        }

        void
        HeatPumpWiFi::setup()
        {
            ESP_LOGI(TAG, "HeatPumpWiFi setup");

            ESP_LOGD(TAG, "Flushing UART...");
            this->flush();
            ESP_LOGD(TAG, "UART flushed");

            ESP_LOGD(TAG, "Setup done");
        }

        void HeatPumpWiFi::loop()
        {
            // Read data from UART
            while (available())
            {
                uint8_t byte = read();
                // Parse your protocol here
                ESP_LOGV(TAG, "Received: 0x%02X", byte);
            }
        }

        void HeatPumpWiFi::dump_config()
        {
            ESP_LOGCONFIG(TAG, "HeatPumpWiFi:");
            ESP_LOGE(TAG, "HeatPumpWiFi::dump_config() Test ERROR");
            ESP_LOGW(TAG, "HeatPumpWiFi::dump_config() Test WARN");
            ESP_LOGI(TAG, "HeatPumpWiFi::dump_config() Test INFO");
            ESP_LOGD(TAG, "HeatPumpWiFi::dump_config() Test DEBUG");
        }

        void HeatPumpWiFi::send_command_(uint8_t *data, size_t len)
        {
            write_array(data, len);
            flush();
        }

    } // namespace heatpumpwifi
} // namespace esphome
